...
def main():
    # Đọc dữ liệu gốc (chỉ positive triples)
    ...
    negative_df = create_hard_negative_samples(df)
    
    print(f"Hard Negative samples created: {len(negative_df)}")
    ...
    train_df.to_csv('data/KG_VNT/train.txt', sep='\t', index=False, header=False)
    dev_df.to_csv('data/KG_VNT/dev.txt', sep='\t', index=False, header=False)
    test_df.to_csv('data/KG_VNT/test.txt', sep='\t', index=False, header=False)
    ...
    # In một số ví dụ Hard Negative
    print("\nSample Hard Negative examples:")
    hard_negatives = negative_df.head(5)
    for _, row in hard_negatives.iterrows():
        print(f"Hard Negative: {row['head']} {row['relation']} {row['tail']}")
